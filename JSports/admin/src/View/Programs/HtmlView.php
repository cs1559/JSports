 ...
 *
 */
namespace FP4P\Component\JSports\Administrator\View\Programs;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolBar()
    {
        ...
        if ($canDo->get('core.edit.state'))
        {
            ...
            $childBar->publish('programs.publish')->listCheck(true);
            
            $childBar->unpublish('programs.unpublish')->listCheck(true);
            
            $childBar->archive('programs.archive')->listCheck(true);
            
            if ($this->state->get('filter.published') != -2)
            {
                $childBar->trash('programs.trash')->listCheck(true);
            }
        }
        
        if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
        {
            $toolbar->delete('programs.delete')
